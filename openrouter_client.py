...
class OpenRouterClient:
    ...
    def chat(
        ...
        """
        Send messages directly to the API without any wrapping.
        Used by LightRAG for entity extraction and other internal prompts.
        """
        try:
            payload = {
                "model": model,
            ...
            if 'choices' not in response_data or not response_data['choices']:
                print(f"[DEBUG] No choices in response: {json.dumps(response_data, ensure_ascii=False)[:300]}")
                raise Exception("No response generated by the model")

            assistant_response = response_data['choices'][0]['message'].get('content')

            if assistant_response is None:
                print(f"[DEBUG] content is None. Full message: {json.dumps(response_data['choices'][0]['message'], ensure_ascii=False)[:300]}")
                return ""
    ...
    def get_response(
        ...
        try:
            ...
            if 'choices' in response_data and response_data['choices']:
                ...
                print(f"[DEBUG] Response content length: {len(content) if content else 'None'}")
                print(f"[DEBUG] finish_reason: {response_data['choices'][0].get('finish_reason')}")
            else:
                print(f"[DEBUG] Full API response: {json.dumps(response_data, ensure_ascii=False)[:500]}")
